import { LANG_EN, LANG_RU } from 'constants/LANG'
import React from 'react'

export default {
  left: {
    [LANG_RU]: <div>
      <h4>Эко-туризм</h4>
      <br />

      <p>
        Фестиваль несет следующие идеи:
      </p>
      <ul>
        <li>взаимодействие с&nbsp;природой, созерцания&nbsp;ее, не&nbsp;нарушая ее&nbsp;целостность;</li>
        <li>духовного обогащения от&nbsp;взаимодействия с&nbsp;природой.</li>
      </ul>
      <p></p>
      <p>Цель продвижения экотуризма в&nbsp;формате фестиваля&nbsp;&mdash; это привлечение местных жителей
        и&nbsp;получение ими преимуществ от&nbsp;своей деятельности, обеспечивая нужды фестиваля,
        без ущерба природе с&nbsp;сохранением ценностей местности!</p>

      <p>Привлечение местных жителей для реализации на&nbsp;фестивале здорового экологически
        чистого питания гостей фестиваля и&nbsp;местных товаров ремесла.</p>
    </div>,
    [LANG_EN]: <div>
      <h4>Eco tourism</h4>
      <br />

      <p>The festival spreads the message of&nbsp;interaction with nature and contemplating&nbsp;it, not affecting its
        integrity, encourages spiritual enrichment received from interaction with nature. </p>
      <p></p>
      <p>Within the framework of&nbsp;the festival the aim of&nbsp;promoting ecotourism is&nbsp;to&nbsp;involve locals.
        They can benefit from their activities supporting the festival needs, harmless to&nbsp;the nature and preserving
        the local area.</p>

      <p>We&nbsp;involve local people to&nbsp;provide healthy ecological catering and local crafts for visitors.</p>
    </div>
  },
  right: {
    [LANG_RU]: <div>
      <p><b>Образование</b></p>
      <p>Интересные лекторы, преподаватели, мастера,
        ученые и&nbsp;учителя поделятся с&nbsp;Вами на&nbsp;лекциях и&nbsp;мастер-классах знаниями и&nbsp;опытом,
        интересно расскажут о&nbsp;фундаментальных законах точных и&nbsp;гуманитарных наук.</p>
      <p><b>Здоровье</b></p>
      <p>Фестиваль Sun Spirit пропагандирует осознанный и&nbsp;здоровый образ жизни.
        Наша команда мастеров здоровья предложит Вам пройти сеансы массажа,
        оздоровительной гимнастики, спа, покажет и&nbsp;проведет при желании различные практики,
        такие как ходьба по&nbsp;углям, лежание на&nbsp;гвоздях, и&nbsp;многое-многое другое.</p>
    </div>,
    [LANG_EN]: <div>
      <p><b>Education</b></p>
      <p>Education Interesting lecturers, teachers, handymen, scientists and tutors will share with you their knowledge
        and skills, tell about fundamental laws of&nbsp;science and arts at&nbsp;the lectures and master-classes.</p>
      <p><b>Health</b></p>
      <p>The festival Sun Spirit encourages conscious and healthy lifestyle. Our team of&nbsp;health specialists
        will offer you massage sessions, recreational gymnastics and spa, they will show you and conduct different
        practices like firewalk, lying on&nbsp;a&nbsp;bed of&nbsp;nails and what&rsquo;s not.</p>
    </div>
  }
}
